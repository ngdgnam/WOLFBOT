...
module.exports = function ({ api, Threads }) {
  cron.schedule('*/10 * * * *', async () => {
    ...
    if (fs.existsSync(cc)) {
        const { datetime } = (() => { if (!fs.existsSync(cc, 'utf-8')) (() => { const dir = path.dirname(cc); if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true }); return fs.writeFileSync(cc, 'utf-8', JSON.stringify({}, null, 2)); return JSON.parse(fs.readFileSync(cc, 'utf-8')); })();
    ...
    if (!lastRunTime || moment(currentTime).diff(moment(lastRunTime), 'minutes') >= 10) {
        ...
        if (dataChanged) {
            (() => { const dir = path.dirname(cc); if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true }); return fs.writeFileSync(cc, JSON.stringify({ datetime: currentTime }));
