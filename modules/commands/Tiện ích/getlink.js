...
module.exports.run = async ({ api, event }) => {
    ...
    if (!event.messageReply.attachments || event.messageReply.attachments.length == 0) return api.sendMessage("❎ Bạn phải reply một audio, video, ảnh nào đó", event.threadID, event.messageID)} else {
    let num = 0
