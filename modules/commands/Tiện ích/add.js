...
this.run = async function  ({ api, event, args, Threads, Users }) {
  ...
  if (link.indexOf(".com/") !== -1) {
    ...
    if (err) return api.sendMessage(`❎ Không thể thêm thành viên vào nhóm`, threadID, messageID)} } else if (approvalMode && !adminIDs.some(item => item.id == api.getCurrentUserID()) return api.sendMessage(`✅ Đã thêm người dùng vào danh sách phê duyệt`, threadID, messageID)} else return api.sendMessage(`✅ Thêm thành viên vào nhóm thành công`, threadID, messageID);
    });
  }
  else {
    var uidUser = args[0]
