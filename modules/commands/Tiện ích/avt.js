...
module.exports.run = async function  ({ api, event, args, Threads }) {
	...
	if (args[0] == "box") {
		if (args[1]) {
			...
			if (!imgg) api.sendMessage(`Avata của box ${threadInfo.threadName} đây`, event.threadID, event.messageID)} else var callback = () => api.sendMessage({ body: `Avata box ${threadInfo.threadName} đây`, attachment: fs.createReadStream(__dirname + "/cache/1.png") }, event.threadID, () => fs.unlinkSync(__dirname + "/cache/1.png"), event.messageID);
