...
if(!isNaN(args[0]) {
    ...
    if (coins > balance) return api.sendMessage('Số tiền bạn muốn chuyển lớn hơn số dư bạn hiện có!',threadID,messageID)} else {
...
if(!mention) return api.sendMessage('Vui long tag người mà bạn cần chuyển tiền cho!',threadID,messageID)} else {
	if(!isNaN(args[0+ name]) {
		const coins = parseInt(args[0+ name]);
		let balance = (await Currencies.getData(senderID).money;
    if (coins <= 0) return api.sendMessage('Số tiền bạn muốn chuyển không hợp lệ',threadID,messageID);
		if (coins > balance) return api.sendMessage('Số tiền của bạn muốn chuyển lớn hơn số dư của bạn hiện có!',threadID,messageID)} else {
