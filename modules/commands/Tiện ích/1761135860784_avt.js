...
if (!args[0]) return api.sendMessage(`${prefix}${mn} box -> get avt của nhóm bạn\n${prefix}${mn} id + id -> get ảnh theo uid người đó\n${prefix}${mn} user -> để trống là get avatar của chính người dùng lệnh\n${prefix}${mn} user + @tag -> get avatar người được tag`,event.threadID,event.messageID);
  if (args[0] == "box") {
    ...
    if(!imgg) api.sendMessage(`Avatar của box ${threadInfo.threadName} đây`,event.threadID,event.messageID)} else var callback = () => api.sendMessage({body:`Avatar box ${threadInfo.threadName} đây`,attachment: fs.createReadStream(__dirname + "/cache/1.png")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/1.png"), event.messageID); 
