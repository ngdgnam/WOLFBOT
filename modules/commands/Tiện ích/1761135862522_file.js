...
module.exports.handleReply = function({ handleReply: $, api, event }) {
    ...
    switch (action) {
    case 'open':
    if (d.info.isDirectory() openFolder(api, event, d.dest)} else api.sendMessage('⚠️ Path not a directory', event.threadID, event.messageID);
    break;
    case 'del': {
    var arrFile = [],
    fo,
    ...
    api.sendMessage({
    attachment: createReadStream(t || p)}, event.threadID, _ => unlinkSync(t), event.messageID);
    };
    break;
    case "create": {
    let t;
...
function size_folder(folder = '') {
    ...
    if (info.isDirectory() bytes += size_folder(path)} else bytes += info.size;
    } catch (){
    continue
    }
