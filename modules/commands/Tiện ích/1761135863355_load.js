...
if (errorList.length != 0) {
  ...
  return;
    ...
    case "mdl": {
    if (moduleList.length == 0) return api.sendMessage(" ➜  Tên module không được để trống!", threadID, messageID)} else return loadCommand({ moduleList, threadID, messageID });
    }
    case "un": {
    if (moduleList.length == 0) return api.sendMessage(" ➜  Tên module không được để trống!", threadID, messageID)} else return unloadModule({ moduleList, threadID,   messageID });
    }
    case "All": {
